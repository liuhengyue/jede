...
from . import build_neck_output
from ..losses import pg_rcnn_loss
from ..utils import ctdet_decode

ROI_NUMBER_NECK_REGISTRY = Registry("ROI_NUMBER_NECK")


@ROI_NUMBER_NECK_REGISTRY.register()
class NumberNeck(nn.Module):
    def __init__(self,
        ...
        """
                ...
                """
        super().__init__()
        neck_cfg = cfg.MODEL.ROI_NUMBER_NECK_OUTPUT
        self.neck_output = build_neck_output(neck_cfg, input_shape)
        self.size_target_type = cfg.MODEL.ROI_NUMBER_NECK_OUTPUT.SIZE_TARGET_TYPE
        self.size_target_scale = cfg.MODEL.ROI_NUMBER_NECK_OUTPUT.SIZE_TARGET_SCALE
        self.out_head_weights = cfg.MODEL.ROI_NUMBER_NECK_OUTPUT.OUTPUT_HEAD_WEIGHTS
        self.add_box_constraints = cfg.MODEL.ROI_NUMBER_NECK_OUTPUT.ADD_BOX_CONSTRAINTS
    ...
    def inference(self, instances, center_heatmaps, scale_heatmaps, offset_heatmaps):
        ...
        for i, boxes in enumerate(detection_boxes):
            ...
            boxes = [boxes_per_ins[inside_matched_box(boxes_per_ins, pred_person_boxes[j])] for j, boxes_per_ins
                     in enumerate(boxes)]
            instances[i].pred_number_boxes = boxes
        return instances
...
def build_number_neck_output(cfg, input_shapes):
    """
    Build a box head defined by `cfg.MODEL.ROI_BOX_HEAD.NAME`.
    """
    name = cfg.MODEL.ROI_NUMBER_NECK.NAME
    if not name:
        return None
    return ROI_NUMBER_NECK_REGISTRY.get(name)(cfg, input_shapes)